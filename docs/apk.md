...
CSS / WebAssembly（zxing）・OCRエンジン（ML Kit、Androidネイティブ依存として
静的リンク）・アイコンまで、すべて APK の中に同梱されています。ネットワーク通信の許可（`INTERNET`
...
**現在、OCR（文字モード）は APK 版でのみ動作します。** OCRエンジンは
Google ML Kit（`com.google.mlkit:text-recognition`、Androidのネイティブ
依存として APK に静的リンク）の1本で、Capacitor のネイティブプラグイン
経由でしか呼び出せないためです。Web版（`pnpm dev` / GitHub Pages）では
ML Kit のプラグインが存在せず、文字モードのシャッターボタンは無効化され、
「OCRはAndroidアプリ版でのみ利用できます」という案内が表示されます
（バーコードモードは Web版でも従来通り動作します）。
