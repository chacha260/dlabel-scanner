      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        ...
        <Section title="バーコードモード">
          ...
          <ul className="list-disc space-y-1.5 pl-5">
            ...
            <li>
              <strong className="text-slate-100">誤読対策</strong>
              の<ButtonRef>誤読ガードを設定</ButtonRef>
              : バーコードが切れている等の際に別の値を読み取ってしまうことを防ぐ設定です。次のセクションで詳しく説明します。
            </li>
          </ul>
        ...
        <Section title="誤読ガード（読み取り値の検証）">
          <p>
            バーコードが切れている・かすれている等の際に、
            <strong className="text-slate-100">
              規格上は妥当だが実際とは違う値を読み取ってしまう
            </strong>
            ことがあります。<ButtonRef>誤読ガードを設定</ButtonRef>
            から、この誤読を止めるための検証を設定できます。
            <strong className="text-amber-300">
              QR・DataMatrix・PDF417（2次元シンボル）には適用されません
            </strong>
            （強力な誤り訂正を内蔵しており、デコードが成功した時点で内容が保証されているためです）。
          </p>
          <ul className="list-disc space-y-1.5 pl-5">
            <li>
              <strong className="text-slate-100">有効なシンボロジー</strong>
              : 現場のラベルで実際に使っているフォーマットだけに絞り込めます（既定は全種類ON）。チェックを外したフォーマットは、正しく読めていても採用しません。
            </li>
            <li>
              <strong className="text-slate-100">チェックディジットの検算</strong>
              （既定でGTIN系はON）: EAN-13・EAN-8・UPC-A・UPC-E・ITFは、読み取った値のチェックディジットを検算し、合わないものは読み取りません。
              <strong className="text-slate-100">ITFは14桁（ITF-14/GTIN-14）のときだけ</strong>
              検算します。それ以外の桁数のITFはチェックディジットを持たない運用があるため対象外です。Code39の検算は規格上「任意」で付けていない運用の方が多いため既定はOFFです。Code128はチェックディジットがシンボロジー自体に内蔵されているため設定自体がありません。
            </li>
            <li>
              <strong className="text-slate-100">見切れ検出</strong>
              （既定でON）: 検出したバーコードの枠が画像の縁ぎりぎりにある場合、見切れているとみなして読み取りません。「枠内のみ」がONのときは、水色の枠から少しでもはみ出したバーコードが見切れ扱いになります。
            </li>
            <li>
              <strong className="text-slate-100">桁数の範囲</strong>
              （既定は無制限）: 読み取った値の文字数の下限・上限を設定できます。部分読みは本来より短い値になることが多いため、下限の設定が特に効きます。
            </li>
            <li>
              <strong className="text-slate-100">複数回一致</strong>
              （既定2回）: 同じ値を連続してこの回数デコードできて初めて一覧に追加します。1回だけの偶発的な誤読に効きます（10fpsなので2回でも遅延は約100msです）。
              <strong className="text-slate-100">
                複数のバーコードが同時に画角に写っていても、値ごとに独立して数えます
              </strong>
              。ただし、切れたバーコードを毎フレーム同じように誤読する場合には効きません（どのフレームでも同じ値が出るため）。そのようなケースにはチェックディジットの検算と見切れ検出が効きます。
            </li>
          </ul>
          <p>
            読み取りが棄却されたときは、
            <strong className="text-slate-100">
              「チェックディジットが合いません」「バーコードが見切れています」
            </strong>
            のように、次に何をすればよいか分かる短いメッセージが画面下に表示されます（同じ理由が連続で出続けないよう、一定時間は同じ通知を繰り返しません）。「複数回一致がまだ規定回数に満たない」だけの状態は、次のフレームで採用されうるだけなので通知は出ません。
          </p>
        </Section>
